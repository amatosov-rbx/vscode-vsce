...
function collectAllFiles(cwd, manifest, useYarn = false, dependencyEntryPoints) {
    return npm_1.getDependencies(cwd, manifest, useYarn, dependencyEntryPoints).then(deps => {
        const promises = deps.map(dep => {
            return glob('**', { cwd: dep.src, nodir: true, dot: true, ignore: 'node_modules/**' }).then(files => files.map(f => {
                return {
...
function collectFiles(cwd, manifest, useYarn = false, dependencyEntryPoints, ignoreFile) {
    return collectAllFiles(cwd, manifest, useYarn, dependencyEntryPoints).then(files => {
        ...
        return (readFile(ignoreFile ? ignoreFile : path.join(cwd, '.vscodeignore'), 'utf8')
            ...
            // Filter out files
            .then(({ ignore, negate }) => files.filter(f => !ignore.some(i => minimatch(f.src, i, MinimatchOptions)) ||
